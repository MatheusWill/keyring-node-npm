...
async function defineModel({
  ...
  Keyring(model, { keys, columns, encryption, keyringIdColumn, salt });
...
suite("sequelize", () => {
  ...
  test("should returns uninitialized values", async () => {
  ...
  test("should handles non-string values", async () => {
    const User = await defineModel({
      keys: { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" },
    });
  ...
  test("should encrypts multiple attributes", async () => {
    const User = await defineModel({
      keys: { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" },
    });
  ...
  test("should loads one record", async () => {
    const User = await defineModel({
      keys: { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" },
    });
  ...
  test("should loads several records", async () => {
    const User = await defineModel({
      keys: { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" },
    });
  ...
  test("should updates record", async () => {
    const User = await defineModel({
      keys: { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" },
    });
    ...
    assert.equal(user.email_digest, sha1("NEW EMAIL", { salt: "" }));
    assert.equal(user.secret, "NEW SECRET");
  });

  test("should saves keyring id", async () => {
    const User = await defineModel({
      keys: { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" },
    });
  ...
  test("should sets digest for existing columns", async () => {
    const keys = { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" };
    const User = await defineModel({ keys });
    const user = await User.create({ email: "EMAIL", secret: "SECRET" });
    ...
    assert.equal(user.email_digest, sha1("EMAIL", { salt: "" }));
    assert.equal(user.secret_digest, undefined);
  });

  test("should uses custom keyring id column", async () => {
    const keys = { 1: "uDiMcWVNTuz//naQ88sOcN+E40CyBRGzGTT7OkoBS6M=" };
  ...
  test("should encrypts attributes using custom encryption", async () => {
    const keys = {
